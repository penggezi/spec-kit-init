...
>
> ⚡ **并行提示**：此步骤与 1.3（代码库分析）互不依赖，可并行执行。优先使用后台任务模式启动本步骤，在等待期间执行 1.3 的代码分析，最后等待本步骤完成。
...
```
第一波（文件创建 || Bug Extension 安装）：
  5.1 安装 Bug Extension（网络耗时，优先后台执行）
  ← 同时 → 3.1 创建经验文件 → 3.2 安装 /retro（含 3.2.1）→ 4.1 安装 /speckit-quality
  → 确认 5.1 完成后再继续
...
第三波（质量门禁注入 || Bug Extension 验证）：
  4.2 改造 /speckit-implement（质量门禁）  ← 同时 →  5.2 验证 Bug Extension
  （4.2 依赖 3.4 的注入状态，5.2 依赖 5.1 的安装结果，两者之间无依赖）
  → 然后顺序执行 5.3（依赖 5.2）→ 5.4
...
#### 5.1 安装官方 Bug Extension ⚡第一波

> 此步骤在阶段 3-5 并行编排的「第一波」中执行，与 3.1/3.2/4.1 的文件操作互不依赖，可并行执行。优先使用后台任务模式启动此命令，在等待期间执行文件创建操作。

在阶段 2 完成后立即启动。告知用户：正在安装官方 Spec-Kit Bug Extension，随后可使用三步缺陷修复流程（assess → fix → test）。
