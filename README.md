...
项目初始化与 SDD 规范一体化工具——Claude Code Skill，替代内置 `/init` 命令，在一次初始化中同时完成五件事：
...
| 文件 | 角色 | 被读取时机 |
|------|------|-----------|
| `SKILL.md` | **主入口**，定义 6 个初始化阶段的完整流程 | 每次会话必读 |
...
`/speckit-quality` 的默认目标是本次改动，而不是全项目存量治理：

```bash
/speckit-quality                    # Git 变更 + 有证据的直接影响范围
/speckit-quality src/api/order.ts   # 显式文件范围
/speckit-quality module:services    # 显式模块范围
/speckit-quality --all              # 显式全量检查
```

新增、修改和未追踪的源文件/测试文件会被纳入检查；删除文件只用于判断影响范围，不会直接传给 lint。修改模块入口、公共类型、路由/注册或模块配置时升级模块级；根依赖/锁文件、构建/质量配置、数据库迁移、跨模块契约或影响边界不明时升级全量。报告会把本次变更问题、关联存量问题和无法归因问题分开，未修改区域的历史问题默认不阻塞本次工作。
...
### Bug 修复流程（安装官方 Bug Extension 后）

| 命令 | 作用 | 产出 |
|------|------|------|
| `/speckit.bug.assess` | 根据缺陷描述评估问题、定位可疑代码路径并提出修复建议 | `.specify/bugs/<slug>/assessment.md` |
| `/speckit.bug.fix` | 应用评估后的最小修复 | `.specify/bugs/<slug>/fix.md` |
| `/speckit.bug.test` | 重测缺陷、执行相关验证并记录结果 | `.specify/bugs/<slug>/test.md` |
...
Skill 触发后，按 6 个阶段依次执行，中间会在关键决策点（AI 工具选择、Constitution 初始化）与用户交互确认。Bug Extension 会在 Claude Code / Codex 项目中自动安装。
