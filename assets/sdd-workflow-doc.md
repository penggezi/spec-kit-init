...
- **quality** 为可选代码质量检查（`/speckit-quality`）：需要时手动执行，默认检查 Git 变更及有证据的直接影响范围；仅在风险、影响边界或用户要求触发时扩大到模块或全量
...
**作用**：按 tasks.md 顺序执行编码工作，逐项完成并标记进度。完成后主动询问是否进行 `/retro` 复盘。
